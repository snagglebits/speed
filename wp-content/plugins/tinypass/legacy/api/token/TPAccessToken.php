...
class TPAccessToken {
	...
	public function incrementViewCount() {
		if ($this->getMeterType() != TPAccessToken::METER_STRICT && $this->isMeterViewBased()) {
			if ($this->getTrialViewCount() == $this->getTrialViewLimit()) {
				$this->token->addField(
								TPTokenData::METER_LOCKOUT_ENDTIME,
								time() +
								(TPUtils::parseLoosePeriodInMsecs( $this->token->getField(TPTokenData::METER_LOCKOUT_PERIOD))/1000)
				);
			}
		}
		$this->token->addField(TPTokenData::METER_TRIAL_ACCESS_ATTEMPTS, $this->getTrialViewCount() + 1);
	}
	*/
